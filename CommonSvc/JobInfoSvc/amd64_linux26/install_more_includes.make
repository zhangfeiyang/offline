...
#cmt_local_tagfile_install_more_includes = $(JobInfoSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(JobInfoSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)JobInfoSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)JobInfoSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "JobInfoSvc" = ""; then \
	    if test -d ../JobInfoSvc; then \
	      echo "Installing files from more ../JobInfoSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/JobInfoSvc; then mkdir -p $(install_include_dir)/JobInfoSvc; fi;\
	        $(install_command) ../JobInfoSvc $(install_include_dir);\
	    else \
	      echo "No more JobInfoSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/JobInfoSvc"; \
	  $(uninstall_command) "$(install_include_dir)/JobInfoSvc/*" ; \
	  if test -f $(install_include_dir)/JobInfoSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/JobInfoSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/JobInfoSvc.cmtref ; \
	  fi; \
	fi
