...
#cmt_local_tagfile_install_more_includes = $(RandomSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RandomSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RandomSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RandomSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RandomSvc" = ""; then \
	    if test -d ../RandomSvc; then \
	      echo "Installing files from more ../RandomSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RandomSvc; then mkdir -p $(install_include_dir)/RandomSvc; fi;\
	        $(install_command) ../RandomSvc $(install_include_dir);\
	    else \
	      echo "No more RandomSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RandomSvc"; \
	  $(uninstall_command) "$(install_include_dir)/RandomSvc/*" ; \
	  if test -f $(install_include_dir)/RandomSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/RandomSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/RandomSvc.cmtref ; \
	  fi; \
	fi
