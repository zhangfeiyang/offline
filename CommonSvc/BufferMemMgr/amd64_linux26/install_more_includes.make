...
#cmt_local_tagfile_install_more_includes = $(BufferMemMgr_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(BufferMemMgr_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)BufferMemMgr_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)BufferMemMgrsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "BufferMemMgr" = ""; then \
	    if test -d ../BufferMemMgr; then \
	      echo "Installing files from more ../BufferMemMgr to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/BufferMemMgr; then mkdir -p $(install_include_dir)/BufferMemMgr; fi;\
	        $(install_command) ../BufferMemMgr $(install_include_dir);\
	    else \
	      echo "No more BufferMemMgr include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/BufferMemMgr"; \
	  $(uninstall_command) "$(install_include_dir)/BufferMemMgr/*" ; \
	  if test -f $(install_include_dir)/BufferMemMgr.cmtref ; then \
	    echo "Removing $(install_include_dir)/BufferMemMgr.cmtref"; \
	    eval rm -f $(install_include_dir)/BufferMemMgr.cmtref ; \
	  fi; \
	fi
