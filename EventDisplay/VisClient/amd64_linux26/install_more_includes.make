...
#cmt_local_tagfile_install_more_includes = $(VisClient_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(VisClient_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)VisClient_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)VisClientsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "VisClient" = ""; then \
	    if test -d ../VisClient; then \
	      echo "Installing files from more ../VisClient to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/VisClient; then mkdir -p $(install_include_dir)/VisClient; fi;\
	        $(install_command) ../VisClient $(install_include_dir);\
	    else \
	      echo "No more VisClient include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/VisClient"; \
	  $(uninstall_command) "$(install_include_dir)/VisClient/*" ; \
	  if test -f $(install_include_dir)/VisClient.cmtref ; then \
	    echo "Removing $(install_include_dir)/VisClient.cmtref"; \
	    eval rm -f $(install_include_dir)/VisClient.cmtref ; \
	  fi; \
	fi
