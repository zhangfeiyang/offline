...
#cmt_local_tagfile_install_more_includes = $(JVisLib_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(JVisLib_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)JVisLib_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)JVisLibsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "JVisLib" = ""; then \
	    if test -d ../JVisLib; then \
	      echo "Installing files from more ../JVisLib to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/JVisLib; then mkdir -p $(install_include_dir)/JVisLib; fi;\
	        $(install_command) ../JVisLib $(install_include_dir);\
	    else \
	      echo "No more JVisLib include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/JVisLib"; \
	  $(uninstall_command) "$(install_include_dir)/JVisLib/*" ; \
	  if test -f $(install_include_dir)/JVisLib.cmtref ; then \
	    echo "Removing $(install_include_dir)/JVisLib.cmtref"; \
	    eval rm -f $(install_include_dir)/JVisLib.cmtref ; \
	  fi; \
	fi
