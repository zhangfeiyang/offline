...
#cmt_local_tagfile = $(JVisLib_tag).make
cmt_local_tagfile = $(bin)$(JVisLib_tag).make
...
cmt_local_tagfile_install_more_includes = $(bin)$(JVisLib_tag)_install_more_includes.make
...
cmt_local_tagfile_install_more_includes = $(bin)$(JVisLib_tag).make
...
JVisLib_extratags = -tag_add=target_JVisLib
...
not_JVisLibcompile_dependencies = { n=0; for p in $?; do m=0; for d in $(JVisLibcompile_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
JVisLibdirs :
	@if test ! -d $(bin)JVisLib; then $(mkdir) -p $(bin)JVisLib; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)JVisLib
else
JVisLibdirs : ;
endif
...
ifndef QUICK
$(cmt_local_JVisLib_makefile) : $(JVisLibcompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building JVisLib.make"; \
	  $(cmtexe) -tag=$(tags) $(JVisLib_extratags) build constituent_config -out=$(cmt_local_JVisLib_makefile) JVisLib
else
$(cmt_local_JVisLib_makefile) : $(JVisLibcompile_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_JVisLib) ] || \
	  [ ! -f $(cmt_final_setup_JVisLib) ] || \
	  $(not_JVisLibcompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building JVisLib.make"; \
	  $(cmtexe) -tag=$(tags) $(JVisLib_extratags) build constituent_config -out=$(cmt_local_JVisLib_makefile) JVisLib; \
	  fi
endif
...
$(cmt_local_JVisLib_makefile) : $(JVisLibcompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building JVisLib.make"; \
	  $(cmtexe) -f=$(bin)JVisLib.in -tag=$(tags) $(JVisLib_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_JVisLib_makefile) JVisLib
else
$(cmt_local_JVisLib_makefile) : $(JVisLibcompile_dependencies) $(cmt_build_library_linksstamp) $(bin)JVisLib.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_JVisLib) ] || \
	  [ ! -f $(cmt_final_setup_JVisLib) ] || \
	  $(not_JVisLibcompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(JVisLib_extratags) build constituent_makefile -out=$(cmt_local_JVisLib_makefile) JVisLib

JVisLib :: JVisLibcompile JVisLibinstall ;

ifdef cmt_JVisLib_has_prototypes

JVisLibprototype : $(JVisLibprototype_dependencies) $(cmt_local_JVisLib_makefile) dirs JVisLibdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_JVisLib_makefile); then \
	  $(MAKE) -f $(cmt_local_JVisLib_makefile) $@; \
	  fi
#	@$(MAKE) -f $(cmt_local_JVisLib_makefile) $@
	$(echo) "(constituents.make) $@ done"

JVisLibcompile : JVisLibprototype

endif

JVisLibcompile : $(JVisLibcompile_dependencies) $(cmt_local_JVisLib_makefile) dirs JVisLibdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_JVisLib_makefile); then \
	  $(MAKE) -f $(cmt_local_JVisLib_makefile) $@; \
	  fi
...
JVisLibclean :: $(JVisLibclean_dependencies) ##$(cmt_local_JVisLib_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_JVisLib_makefile); then \
	  $(MAKE) -f $(cmt_local_JVisLib_makefile) $@; \
	fi
	$(echo) "(constituents.make) $@ done"
#	@-$(MAKE) -f $(cmt_local_JVisLib_makefile) JVisLibclean

##	  /bin/rm -f $(cmt_local_JVisLib_makefile) $(bin)JVisLib_dependencies.make

install :: JVisLibinstall ;

JVisLibinstall :: JVisLibcompile $(JVisLib_dependencies) $(cmt_local_JVisLib_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_JVisLib_makefile); then \
	  $(MAKE) -f $(cmt_local_JVisLib_makefile) $@; \
	  fi
...
$(foreach d,$(JVisLib_dependencies),$(eval $(d)uninstall_dependencies += JVisLibuninstall))

JVisLibuninstall : $(JVisLibuninstall_dependencies) ##$(cmt_local_JVisLib_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_JVisLib_makefile); then \
	  $(MAKE) -f $(cmt_local_JVisLib_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_JVisLib_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: JVisLibuninstall ;

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(constituents.make) Starting $@ JVisLib"
	$(echo) Using default action for $@
	$(echo) "(constituents.make) $@ JVisLib done"
endif
...
cmt_local_tagfile_make = $(bin)$(JVisLib_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(JVisLib_tag).make
