...
#cmt_local_tagfile_install_more_includes = $(GlobalTimeSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GlobalTimeSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GlobalTimeSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GlobalTimeSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GlobalTimeSvc" = ""; then \
	    if test -d ../GlobalTimeSvc; then \
	      echo "Installing files from more ../GlobalTimeSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GlobalTimeSvc; then mkdir -p $(install_include_dir)/GlobalTimeSvc; fi;\
	        $(install_command) ../GlobalTimeSvc $(install_include_dir);\
	    else \
	      echo "No more GlobalTimeSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GlobalTimeSvc"; \
	  $(uninstall_command) "$(install_include_dir)/GlobalTimeSvc/*" ; \
	  if test -f $(install_include_dir)/GlobalTimeSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/GlobalTimeSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/GlobalTimeSvc.cmtref ; \
	  fi; \
	fi
