...
#cmt_local_tagfile_install_more_includes = $(PmtParamSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(PmtParamSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)PmtParamSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)PmtParamSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "PmtParamSvc" = ""; then \
	    if test -d ../PmtParamSvc; then \
	      echo "Installing files from more ../PmtParamSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/PmtParamSvc; then mkdir -p $(install_include_dir)/PmtParamSvc; fi;\
	        $(install_command) ../PmtParamSvc $(install_include_dir);\
	    else \
	      echo "No more PmtParamSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/PmtParamSvc"; \
	  $(uninstall_command) "$(install_include_dir)/PmtParamSvc/*" ; \
	  if test -f $(install_include_dir)/PmtParamSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/PmtParamSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/PmtParamSvc.cmtref ; \
	  fi; \
	fi
