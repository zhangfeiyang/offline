...
#cmt_local_tagfile_install_more_includes = $(RootRandomSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RootRandomSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RootRandomSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RootRandomSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RootRandomSvc" = ""; then \
	    if test -d ../RootRandomSvc; then \
	      echo "Installing files from more ../RootRandomSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RootRandomSvc; then mkdir -p $(install_include_dir)/RootRandomSvc; fi;\
	        $(install_command) ../RootRandomSvc $(install_include_dir);\
	    else \
	      echo "No more RootRandomSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RootRandomSvc"; \
	  $(uninstall_command) "$(install_include_dir)/RootRandomSvc/*" ; \
	  if test -f $(install_include_dir)/RootRandomSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/RootRandomSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/RootRandomSvc.cmtref ; \
	  fi; \
	fi
