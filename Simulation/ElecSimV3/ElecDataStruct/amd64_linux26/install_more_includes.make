...
#cmt_local_tagfile_install_more_includes = $(ElecDataStruct_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(ElecDataStruct_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)ElecDataStruct_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)ElecDataStructsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "ElecDataStruct" = ""; then \
	    if test -d ../ElecDataStruct; then \
	      echo "Installing files from more ../ElecDataStruct to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/ElecDataStruct; then mkdir -p $(install_include_dir)/ElecDataStruct; fi;\
	        $(install_command) ../ElecDataStruct $(install_include_dir);\
	    else \
	      echo "No more ElecDataStruct include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/ElecDataStruct"; \
	  $(uninstall_command) "$(install_include_dir)/ElecDataStruct/*" ; \
	  if test -f $(install_include_dir)/ElecDataStruct.cmtref ; then \
	    echo "Removing $(install_include_dir)/ElecDataStruct.cmtref"; \
	    eval rm -f $(install_include_dir)/ElecDataStruct.cmtref ; \
	  fi; \
	fi
