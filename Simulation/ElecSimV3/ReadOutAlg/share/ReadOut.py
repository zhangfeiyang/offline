...
def get_parser():
    ...
    parser.add_argument("--circulation", action="append", # default is 0.
                                   help="Whether circularly for TAG,0 for enable circulation,1 for disable. example: --circulation IBD:1 --circulation U:0")
...
def setup_EvtMixing(task):
    ...
    mixAlg.property("CirculationMap").set(circulationMap)
...
if __name__ == "__main__":
    ...
    for c in args.circulation:
        kv = c.split(":")
        if len(kv) == 1: kv.insert(0,"default")
        k, v = kv
        if circulationlist.has_key(k):
            print "ERROR: Don't allow specify different circulation option for same tag"
            import sys 
            sys.exit(-1)
        circulationlist[k] = int(v)
    #print filelists
    #print ratelists
    ...
    for tag, fl in filelists.iteritems():
        ...
        if circulationlist.has_key(tag):
            d["circulation"] = circulationlist[tag]
    ...
    for i in range(sampleNum):
        ...
        if samples[i].has_key("circulation"):
            circulationMap[key_sample_name] = samples[i]["circulation"]
