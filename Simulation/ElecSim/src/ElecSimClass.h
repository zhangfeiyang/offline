...
class Hit_Collection{
    ...
    public:
        void reset();
        void create_vector(int nPhotons,vector<int>& pmtID,vector<double>& hitTime, double Sig_evt_GlobalTime, double split_evt_time);
        vector<Hit>& get_sub_vector(int i);
        double get_earliest();
        double get_latest();
        map<int, vector<Hit> >& get_hit_map();

        void check_hitTime(int pmtID, int map_num, double earliest_item);

        void create_vector_with_certain_pe_per_pmt(int PmtTotal, int pe_num_per_pmt, double Sig_evt_GlobalTime) ;

    private:  
        vector<Hit> total_hit_vector; //contain the all hits of one event
        map<int, vector<Hit> > hit_map; //key is the subEvtId, value is the hit vector
        double earliest_item;
        double latest_item;
};
