...
void Hit_Collection::create_vector(int nPhotons,vector<int>& pmtID,vector<double>& hitTime){


    for(int i=0; i<nPhotons; i++){
         double o_hitTime = hitTime[i];  // unit ns
...
//    cout<<"nPhotons: " <<nPhotons<<endl;
    std::sort(hit_vector.begin(),hit_vector.end(), SortByhitTime);

}

vector<Hit>& Hit_Collection::get_vector(){
    return hit_vector;
}
