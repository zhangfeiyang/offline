...
#cmt_local_tagfile = $(G4Svc_tag).make
cmt_local_tagfile = $(bin)$(G4Svc_tag).make
...
G4Svc_extratags = -tag_add=target_G4Svc
...
not_G4Svccompile_dependencies = { n=0; for p in $?; do m=0; for d in $(G4Svccompile_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
G4Svcdirs :
	@if test ! -d $(bin)G4Svc; then $(mkdir) -p $(bin)G4Svc; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)G4Svc
else
G4Svcdirs : ;
endif
...
ifndef QUICK
$(cmt_local_G4Svc_makefile) : $(G4Svccompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building G4Svc.make"; \
	  $(cmtexe) -tag=$(tags) $(G4Svc_extratags) build constituent_config -out=$(cmt_local_G4Svc_makefile) G4Svc
else
$(cmt_local_G4Svc_makefile) : $(G4Svccompile_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_G4Svc) ] || \
	  [ ! -f $(cmt_final_setup_G4Svc) ] || \
	  $(not_G4Svccompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building G4Svc.make"; \
	  $(cmtexe) -tag=$(tags) $(G4Svc_extratags) build constituent_config -out=$(cmt_local_G4Svc_makefile) G4Svc; \
	  fi
endif
...
$(cmt_local_G4Svc_makefile) : $(G4Svccompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building G4Svc.make"; \
	  $(cmtexe) -f=$(bin)G4Svc.in -tag=$(tags) $(G4Svc_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_G4Svc_makefile) G4Svc
else
$(cmt_local_G4Svc_makefile) : $(G4Svccompile_dependencies) $(cmt_build_library_linksstamp) $(bin)G4Svc.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_G4Svc) ] || \
	  [ ! -f $(cmt_final_setup_G4Svc) ] || \
	  $(not_G4Svccompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(G4Svc_extratags) build constituent_makefile -out=$(cmt_local_G4Svc_makefile) G4Svc

G4Svc :: G4Svccompile G4Svcinstall ;

ifdef cmt_G4Svc_has_prototypes

G4Svcprototype : $(G4Svcprototype_dependencies) $(cmt_local_G4Svc_makefile) dirs G4Svcdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_G4Svc_makefile); then \
	  $(MAKE) -f $(cmt_local_G4Svc_makefile) $@; \
	  fi
#	@$(MAKE) -f $(cmt_local_G4Svc_makefile) $@
	$(echo) "(constituents.make) $@ done"

G4Svccompile : G4Svcprototype

endif

G4Svccompile : $(G4Svccompile_dependencies) $(cmt_local_G4Svc_makefile) dirs G4Svcdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_G4Svc_makefile); then \
	  $(MAKE) -f $(cmt_local_G4Svc_makefile) $@; \
	  fi
...
G4Svcclean :: $(G4Svcclean_dependencies) ##$(cmt_local_G4Svc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_G4Svc_makefile); then \
	  $(MAKE) -f $(cmt_local_G4Svc_makefile) $@; \
	fi
	$(echo) "(constituents.make) $@ done"
#	@-$(MAKE) -f $(cmt_local_G4Svc_makefile) G4Svcclean

##	  /bin/rm -f $(cmt_local_G4Svc_makefile) $(bin)G4Svc_dependencies.make

install :: G4Svcinstall ;

G4Svcinstall :: G4Svccompile $(G4Svc_dependencies) $(cmt_local_G4Svc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_G4Svc_makefile); then \
	  $(MAKE) -f $(cmt_local_G4Svc_makefile) $@; \
	  fi
...
$(foreach d,$(G4Svc_dependencies),$(eval $(d)uninstall_dependencies += G4Svcuninstall))

G4Svcuninstall : $(G4Svcuninstall_dependencies) ##$(cmt_local_G4Svc_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_G4Svc_makefile); then \
	  $(MAKE) -f $(cmt_local_G4Svc_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_G4Svc_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: G4Svcuninstall ;

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(constituents.make) Starting $@ G4Svc"
	$(echo) Using default action for $@
	$(echo) "(constituents.make) $@ G4Svc done"
endif
...
cmt_local_tagfile_make = $(bin)$(G4Svc_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(G4Svc_tag).make
