...
// ********************************************************************
//
// $Id: G4DAEReadParamvol.cc,v 1.10.2.1 2009/03/03 10:55:46 gcosmo Exp $
// GEANT4 tag $Name: geant4-09-02-patch-01 $
//
// class G4DAEReadParamvol Implementation
//
// History:
...
void G4DAEReadParamvol::
Box_dimensionsRead( const xercesc::DOMElement* const element,
                    G4DAEParameterisation::PARAMETER& parameter )
{
   G4double lunit = 1.0;
...
ParametersRead(const xercesc::DOMElement* const element) {
        ...
        iter != 0; iter = iter->getNextSibling())
   {
      ...
      else
      {
        ...
        G4Exception("G4DAEReadParamvol::ParametersRead()", "ReadError",
...
ParameterisedRead(const xercesc::DOMElement* const element)
{
        ...
        iter != 0; iter = iter->getNextSibling())
   {
     ...
     if (tag=="parameters")
     {
      ...
      {
        if (tag=="loop") { LoopRead(child,&G4DAERead::Paramvol_contentRead); }
      }
    }
}
...
ParamvolRead(const xercesc::DOMElement* const element, G4LogicalVolume* mother)
{
   ...
   parameterisation = new G4DAEParameterisation();
   ...
   {
     G4Exception("G4DAEReadParamvol::ParamvolRead()",
