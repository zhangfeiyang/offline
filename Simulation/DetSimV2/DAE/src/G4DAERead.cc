...
// ********************************************************************
//
// $Id: G4DAERead.cc,v 1.40.2.1 2009/03/03 10:55:46 gcosmo Exp $
// GEANT4 tag $Name: geant4-09-02-patch-01 $
//
// class G4DAERead Implementation
//
// History:
// - Created.                                  Zoltan Torzsok, November 2007
// -------------------------------------------------------------------------

#include "G4DAERead.hh"
...
G4String G4DAERead::Transcode(const XMLCh* const toTranscode)
{
...
void G4DAERead::OverlapCheck(G4bool flag)
{
   check = flag;
}

G4String G4DAERead::GenerateName(const G4String& nameIn, G4bool strip)
{
...
G4String G4DAERead::GenerateUniqueName(const G4String& nameIn, G4bool strip)
{
   G4String nameOut(nameIn);
...
void G4DAERead::GeneratePhysvolName(const G4String& nameIn,
...
G4String G4DAERead::Strip(const G4String& name) const
{
  G4String sname(name);
  return sname.remove(sname.find("0x"));
}

void G4DAERead::StripName(G4String& name) const
{
  name.remove(name.find("0x"));
}

void G4DAERead::StripNames() const
{
  ...
  G4cout << "Stripping off DAE names of materials, solids and volumes ..."
         << G4endl;
...
void G4DAERead::LoopRead(const xercesc::DOMElement* const element,
     void(G4DAERead::*func)(const xercesc::DOMElement* const))
{
   G4String var;
   G4String from;
   G4String to;
   ...
   if (var.empty())
   {
     G4Exception("G4DAERead::loopRead()", "InvalidRead",
...
void G4DAERead::ExtensionRead(const xercesc::DOMElement* const)
{
   G4String error_msg = "No handle to user-code for parsing extensions!";
   G4Exception("G4DAERead::ExtensionRead()",
               "NotImplemented", JustWarning, error_msg);
}

void G4DAERead::Read(const G4String& fileName,
                            G4bool validation,
                            G4bool isModule)
{
   if (isModule)
   {
      G4cout << "G4DAE: Reading module '" << fileName << "'..." << G4endl;
   }
   else
   {
      G4cout << "G4DAE: Reading '" << fileName << "'..." << G4endl;
   }

   inLoop = 0;
   validate = validation;

   xercesc::ErrorHandler* handler = new G4DAEErrorHandler(!validate);
   ...
   try { parser->parse(fileName.c_str()); }
     ...
     { G4cout << "G4DAE: " << Transcode(e.getMessage()) << G4endl; }
   ...
   if (!doc)
   {
     ...
     G4Exception("G4DAERead::Read()", "InvalidRead",
   ...
   if (isModule)
   {
      G4cout << "G4DAE: Reading module '" << fileName << "' done!" << G4endl;
   }
   else
   {
      G4cout << "G4DAE: Reading '" << fileName << "' done!" << G4endl;
      StripNames();
   }
}
