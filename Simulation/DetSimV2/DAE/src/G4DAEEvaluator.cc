...
//
//
// $Id: G4DAEEvaluator.cc,v 1.20.2.1 2009/03/03 10:55:46 gcosmo Exp $
// GEANT4 tag $ Name:$
//
// class G4DAEEvaluator Implementation
//
...
void G4DAEEvaluator::DefineConstant(const G4String& name, G4double value)
{
   if (eval.findVariable(name))
   {
     G4String error_msg = "Redefinition of constant or variable: "+name;
     G4Exception("G4DAEEvaluator::DefineConstant()", "InvalidExpression",
                 FatalException, error_msg);
   }
   eval.setVariable(name.c_str(),value);
}

void G4DAEEvaluator::DefineVariable(const G4String& name,G4double value)
{
   if (eval.findVariable(name))
   {
     G4String error_msg = "Redefinition of constant or variable: "+name;
     G4Exception("G4DAEEvaluator::DefineVariable()", "InvalidExpression",
                 FatalException, error_msg);
   }
   eval.setVariable(name.c_str(),value);
...
void G4DAEEvaluator::DefineMatrix(const G4String& name,
                                         G4int coldim,
                                         std::vector<G4double> valueList)
{
   ...
   if (size == 0)
   {
     ...
     G4Exception("G4DAEEvaluator::DefineMatrix()", "InvalidSize",
                 FatalException, error_msg);
   }
   if (size == 1)
   {
...
void G4DAEEvaluator::SetVariable(const G4String& name, G4double value)
{
   if (!IsVariable(name))
   {
     G4String error_msg = "Variable '" + name + "' is not defined!";
     G4Exception("G4DAEEvaluator::SetVariable()", "InvalidSetup",
                 FatalException, error_msg);
   }
   eval.setVariable(name.c_str(),value);
}

G4bool G4DAEEvaluator::IsVariable(const G4String& name) const
{
...
G4String G4DAEEvaluator::SolveBrackets(const G4String& in)
{
   ...
   if (open>close)
   {
     ...
     G4Exception("G4DAEEvaluator::SolveBrackets()", "InvalidExpression",
                 FatalException, error_msg);
   }
...
G4double G4DAEEvaluator::Evaluate(const G4String& in)
{
   ...
   if (!expression.empty())
   {
      ...
      if (eval.status() != HepTool::Evaluator::OK)
      {
         ...
         G4Exception("G4DAEEvaluator::Evaluate()", "InvalidExpression",
                     FatalException, error_msg);
      }
   }
   return value;
...
G4int G4DAEEvaluator::EvaluateInteger(const G4String& expression)
{
   ...
   if (frac != 0.0)
   {
     ...
     G4Exception("G4DAEEvaluator::EvaluateInteger()", "InvalidExpression",
                 FatalException, error_msg);
   }
   return whole;
}

G4double G4DAEEvaluator::GetConstant(const G4String& name)
{
   if (IsVariable(name))
   {
     G4String error_msg = "Constant '" + name
                        + "' is not defined! It is a variable!";
     G4Exception("G4DAEEvaluator::GetConstant()", "InvalidSetup",
                 FatalException, error_msg);
   }
   if (!eval.findVariable(name))
   {
...
G4double G4DAEEvaluator::GetVariable(const G4String& name)
{
   if (!IsVariable(name))
   {
     G4String error_msg = "Variable '" + name + "' is not a defined!";
     G4Exception("G4DAEEvaluator::GetVariable()", "InvalidSetup",
                 FatalException, error_msg);
   }
   return Evaluate(name);
}
