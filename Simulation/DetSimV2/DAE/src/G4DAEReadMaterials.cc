...
//
//
// $Id: G4DAEReadMaterials.cc,v 1.16 2008/08/22 10:00:20 gcosmo Exp $
// GEANT4 tag $ Name:$
//
// class G4DAEReadMaterials Implementation
//
// Original author: Zoltan Torzsok, November 2007
//
// --------------------------------------------------------------------

#include "G4DAEReadMaterials.hh"

G4double
G4DAEReadMaterials::AtomRead(const xercesc::DOMElement* const atomElement)
{
   G4double value = 0.0;
...
G4double G4DAEReadMaterials::DRead(const xercesc::DOMElement* const DElement)
{
   G4double value = 0.0;
   G4double unit = g/cm3;
...
G4double G4DAEReadMaterials::PRead(const xercesc::DOMElement* const PElement)
{
...
G4double G4DAEReadMaterials::TRead(const xercesc::DOMElement* const TElement)
{
        ...
        iter != 0; iter = iter->getNextSibling())
   {
      ...
      if (tag=="fraction")
      {
         ...
         if ((materialPtr == 0) && (elementPtr == 0))
         {
            ...
            G4Exception("G4DAEReadMaterials::MixtureRead()", "InvalidSetup",
             ...
             G4Material* material)
{
   ...
   if (matrix.GetCols() != 2)
   {
     G4String error_msg = "Referenced matrix '" + ref
            ...
            + material->GetName();
     G4Exception("G4DAEReadMaterials::PropertyRead()", "InvalidRead",
                 FatalException, error_msg);
   }
...
{
   G4cout << "G4DAE: Reading materials..." << G4endl;
        ...
        iter != 0; iter = iter->getNextSibling())
   {
      ...
      else
      {
        ...
        G4Exception("G4DAEReadMaterials::MaterialsRead()", "InvalidSetup",
                    FatalException, error_msg);
      }
   }
}

G4Element* G4DAEReadMaterials::
GetElement(const G4String& ref, G4bool verbose) const
{
   ...
   if (verbose && !elementPtr)
   {
     ...
     G4Exception("G4DAEReadMaterials::GetElement()", "InvalidRead",
                 FatalException, error_msg);
   }
...
G4Isotope* G4DAEReadMaterials::GetIsotope(const G4String& ref,
                                           G4bool verbose) const
{
   ...
   if (verbose && !isotopePtr)
   {
     ...
     G4Exception("G4DAEReadMaterials::GetIsotope()", "InvalidRead",
                 FatalException, error_msg);
   }
...
G4Material* G4DAEReadMaterials::GetMaterial(const G4String& ref,
                                             G4bool verbose) const
{
   ...
   if (verbose && !materialPtr)
   {
     ...
     G4Exception("G4DAEReadMaterials::GetMaterial()", "InvalidRead",
                 FatalException, error_msg);
   }
