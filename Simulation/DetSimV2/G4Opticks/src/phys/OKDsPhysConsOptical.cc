...
DECLARE_TOOL(OKDsPhysConsOptical);

OKDsPhysConsOptical::OKDsPhysConsOptical(const G4String& name): G4VPhysicsConstructor(name)
                                                            , ToolBase(name)
{
...
OKDsPhysConsOptical::~OKDsPhysConsOptical()
{
}

void OKDsPhysConsOptical::ConstructParticle()
{
}

void OKDsPhysConsOptical::ConstructProcess()
{
#ifdef USE_CUSTOM_CERENKOV
    
    OKDsG4Cerenkov* cerenkov = 0;
    if (m_useCerenkov) {
        cerenkov = new OKDsG4Cerenkov();
...
#ifdef USE_CUSTOM_SCINTILLATION
    OKDsG4Scintillation* scint = 0;
   
    scint = new OKDsG4Scintillation();
