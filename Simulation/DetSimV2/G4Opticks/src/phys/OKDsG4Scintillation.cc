...
//-------------------------------------------------------------------
// OKDsG4Scintillation is a class modified from G4Scintillation
...
// OKDsG4Scintillation::operator=(const OKDsG4Scintillation &right)
// {
// }
...
OKDsG4Scintillation::OKDsG4Scintillation(const G4String& processName,
                                     G4ProcessType type)
    ...
    , alphaSlowerTime(0) , alphaSlowerRatio(0)
{
    ...
    //verboseLevel = 2;
    //G4cout << " OKDsG4Scintillation set verboseLevel by hand to " << verboseLevel << G4endl;
...
OKDsG4Scintillation::~OKDsG4Scintillation() 
{
...
//
G4VParticleChange*
OKDsG4Scintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
    return OKDsG4Scintillation::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
OKDsG4Scintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...
///////////////////////////////////////////////////////////////////////////////
    ...
    G4cout << "S -> material name: " << aMaterial->GetName()
    ...
    int pdgCode = definition->GetPDGEncoding();
    ...
    ss[G4OKScintillationStep::_Id]         =  uifa[0].f ;   // 0
    ss[G4OKScintillationStep::_ParentID]   =  uifa[1].f ;
    ss[G4OKScintillationStep::_Material]   =  uifa[2].f ; 
    ss[G4OKScintillationStep::_NumPhotons] =  uifa[3].f ;

    ss[G4OKScintillationStep::_x0_x] = x0.x() ;             // 1
    ss[G4OKScintillationStep::_x0_y] = x0.y() ;
    ss[G4OKScintillationStep::_x0_z] = x0.z() ;
    ss[G4OKScintillationStep::_t0] = t0 ;

    ss[G4OKScintillationStep::_DeltaPosition_x] = deltaPosition.x(); // 2
    ss[G4OKScintillationStep::_DeltaPosition_y] = deltaPosition.y();
    ss[G4OKScintillationStep::_DeltaPosition_z] = deltaPosition.z();
    ss[G4OKScintillationStep::_step_length]     = aStep.GetStepLength() ;

    ss[G4OKScintillationStep::_code]      =  uifb[0].f ;    // 3
    ss[G4OKScintillationStep::_charge]    =  definition->GetPDGCharge();
    ss[G4OKScintillationStep::_weight]    =  weight ;
    ss[G4OKScintillationStep::_MeanVelocity] = ((pPreStepPoint->GetVelocity()+ pPostStepPoint->GetVelocity())/2.);

    ss[G4OKScintillationStep::_scnt]      =  uifb[1].f ;    // 4
    ss[G4OKScintillationStep::_slowerRatio]  =  slowerRatio ;
    ss[G4OKScintillationStep::_slowTimeConstant]  =  slowTimeConstant ;
    ss[G4OKScintillationStep::_slowerTimeConstant]  =  slowerTimeConstant ;

    ss[G4OKScintillationStep::_ScintillationTime]  = ScintillationTime ;  // 5 
    ss[G4OKScintillationStep::_ScintillationIntegralMax]  = ScintillationIntegral->GetMaxValue() ;
    ss[G4OKScintillationStep::_Spare1]  = 0. ;
    ss[G4OKScintillationStep::_Spare2]  = 0. ;

    daemgr->addGenstep(ss, 24);
...
///////////////////////////////////////////////////////////////////////////////
        ...
        if(scnt == 2) {
            ScintillationTime = slowTimeConstant;
            if(G4UniformRand() < slowerRatio && (!flagReemission)) ScintillationTime = slowerTimeConstant;
        }
...
void OKDsG4Scintillation::BuildThePhysicsTable()
{
...
G4double OKDsG4Scintillation::GetMeanFreePath(const G4Track&,
                                            G4double ,
...
G4double OKDsG4Scintillation::GetMeanLifeTime(const G4Track&,
                                            G4ForceCondition* condition)
{
    *condition = Forced;
