...
OKDsG4Cerenkov::OKDsG4Cerenkov(const G4String& processName, G4ProcessType type)
           : G4VProcess(processName, type)
           , fApplyPreQE(false)
           , fPreQE(1.)
{
        G4cout << "OKDsG4Cerenkov::OKDsG4Cerenkov constructor" << G4endl;
...
//
G4VParticleChange*
OKDsG4Cerenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
	...
	if (verboseLevel>0) {
	  G4cout << "OKDsG4Cerenkov MeanNumberOfPhotons "<< MeanNumberOfPhotons 
...
// IF USING GPU
///////////////////////////////////////////////////////////////////////////////
    static G4OpticksAnaMgr* daemgr = 0;
    if (not daemgr) {
        // get the detsimalg
        SniperPtr<DetSimAlg> detsimalg("DetSimAlg");
        if (detsimalg.invalid()) {
            std::cout << "Can't Load DetSimAlg" << std::endl;
            assert(0);
        }
        ToolBase* t = 0;
        std::string new_name = "G4OpticksAnaMgr";
        t = detsimalg->findTool(new_name);
        if (not t) {
            assert (t);
        }
        daemgr = dynamic_cast<G4OpticksAnaMgr*>(t);
    }

    // == get the Step List ==
    // G4OKCerenkovStepList* csl = daemgr->getCSL();
    // int* g2c = daemgr->getG2C();
    ...
    G4int chromaMaterialIndex = materialIndex;
    // FOR DEBUG: FORCE LS (48)
    // chromaMaterialIndex = 48;

    if (chromaMaterialIndex < 0) {
        G4cout << "chromaMaterialIndex: " << chromaMaterialIndex
               << " material name: " << aMaterial->GetName()
	       << G4endl;
    }

    assert(chromaMaterialIndex > -1);
    G4int chromaBialkaliIndex = 1;

    G4cout << "C -> material name: " << aMaterial->GetName()
           << " -> material idx: " << materialIndex
           << " -> chroma material idx: " << chromaMaterialIndex
           << G4endl;
    ...
    G4ThreeVector deltaPosition = aStep.GetDeltaPosition();
        ...
        // directly fills next item of G4OKArray using (n,4,6) structure [float4 quads efficient on GPU]
    ...
    if (verboseLevel>0) {
        ...
        G4cout << "++ 0 -> " << G4endl;
        G4cout << "+++ _Id: " << uifa[0].i << " -> " << uifa[0].f << G4endl;
        G4cout << "+++ _ParentId: " << uifa[1].i << " -> " << uifa[1].f << G4endl;
        G4cout << "+++ _Materail: " << uifa[2].i << " -> " << uifa[2].f << G4endl;
        G4cout << "+++ _NumPhotons: " << uifa[3].i << " -> " << uifa[3].f << G4endl;

        G4cout << "++ 1 -> " << G4endl;
        G4cout << "+++ _x0_x: " << x0.x() << G4endl;
        G4cout << "+++ _x0_y: " << x0.y() << G4endl;
        G4cout << "+++ _x0_z: " << x0.z() << G4endl;
        G4cout << "+++ _t0: " << t0 << G4endl;

        G4cout << "++ 2 -> " << G4endl;
        G4cout << "+++ _DeltaPosition_x: " << deltaPosition.x() << G4endl;
        G4cout << "+++ _DeltaPosition_y: " << deltaPosition.y() << G4endl;
        G4cout << "+++ _DeltaPosition_z: " << deltaPosition.z() << G4endl;
        G4cout << "+++ _step_length: " << step_length << G4endl;

        G4cout << "++ 3 -> " << G4endl;
        G4cout << "+++ _code: " << uifb[0].i << " -> " << uifb[0].f << G4endl;
        G4cout << "+++ _charge: " << charge << G4endl;
        G4cout << "+++ _weight: " << weight << G4endl;
        G4cout << "+++ _MeanVelocity: " 
               << ((pPreStepPoint->GetVelocity()+pPostStepPoint->GetVelocity())/2.) << G4endl;

        G4cout << "++ 4 -> " << G4endl;
        G4cout << "+++ _BetaInverse: " << BetaInverse << G4endl;
        G4cout << "+++ _Pmin: " << Pmin << G4endl;
        G4cout << "+++ _Pmax: " << Pmax << G4endl;
        G4cout << "+++ _maxCos: " << maxCos << G4endl;

        G4cout << "++ 5 -> " << G4endl;
        G4cout << "+++ _maxSin2: " << maxSin2 << G4endl;
        G4cout << "+++ _MeanNumberOfPhotons1: " << MeanNumberOfPhotons1 << G4endl;
        G4cout << "+++ _MeanNumberOfPhotons2: " << MeanNumberOfPhotons2 << G4endl;
        G4cout << "+++ _BialkaliMaterialIndex: " << uifb[1].i << " -> " << uifb[1].f << G4endl;
    }
    cs[G4OKCerenkovStep::_Id]         =  uifa[0].f ; // 0
    cs[G4OKCerenkovStep::_ParentID]   =  uifa[1].f ;
    cs[G4OKCerenkovStep::_Material]   =  uifa[2].f ; 
    cs[G4OKCerenkovStep::_NumPhotons] =  uifa[3].f ;

    cs[G4OKCerenkovStep::_x0_x] =  x0.x() ;         // 1
    cs[G4OKCerenkovStep::_x0_y] =  x0.y() ;
    cs[G4OKCerenkovStep::_x0_z] =  x0.z() ;
    cs[G4OKCerenkovStep::_t0]   =  t0 ;

    cs[G4OKCerenkovStep::_DeltaPosition_x] =  deltaPosition.x() ;   // 2
    cs[G4OKCerenkovStep::_DeltaPosition_y] =  deltaPosition.y() ;
    cs[G4OKCerenkovStep::_DeltaPosition_z] =  deltaPosition.z() ;
    cs[G4OKCerenkovStep::_step_length] =  step_length ;
 
    cs[G4OKCerenkovStep::_code] = uifb[0].f ;    // 3
    cs[G4OKCerenkovStep::_charge] = charge ;
    cs[G4OKCerenkovStep::_weight] = weight ;
    cs[G4OKCerenkovStep::_MeanVelocity] = ((pPreStepPoint->GetVelocity()+pPostStepPoint->GetVelocity())/2.);

    cs[G4OKCerenkovStep::_BetaInverse] = BetaInverse ;   // 4
    cs[G4OKCerenkovStep::_Pmin] = Pmin ;
    cs[G4OKCerenkovStep::_Pmax] = Pmax ;
    cs[G4OKCerenkovStep::_maxCos] = maxCos ;

    cs[G4OKCerenkovStep::_maxSin2] = maxSin2 ;     // 5
    cs[G4OKCerenkovStep::_MeanNumberOfPhotons1] = MeanNumberOfPhotons1 ;
    cs[G4OKCerenkovStep::_MeanNumberOfPhotons2] = MeanNumberOfPhotons2 ;
    cs[G4OKCerenkovStep::_BialkaliMaterialIndex] = uifb[1].f ; 

    daemgr->addGenstep(cs, 24);
...
///////////////////////////////////////////////////////////////////////////////
	...
	for (G4int i = 0; i < NumPhotons; i++) {
	  ...
	  if (verboseLevel>0) {
	    G4cout << "OKDsG4Cerenkov  aSecondaryTrack->SetWeight( fPhotonWeight="<<fPhotonWeight<<" * aTrack.GetWeight()= " << aTrack.GetWeight() 
	...
	if (verboseLevel>0) {
	G4cout << "\n Exiting from OKDsG4Cerenkov::DoIt -- NumberOfSecondaries = " 
...
void OKDsG4Cerenkov::BuildThePhysicsTable()
{
...
G4double OKDsG4Cerenkov::GetMeanFreePath(const G4Track&,
                                           G4double,
                                           G4ForceCondition*)
{
        return 1.;
...
G4double OKDsG4Cerenkov::PostStepGetPhysicalInteractionLength(
...
G4double 
OKDsG4Cerenkov::GetAverageNumberOfPhotons(const G4double charge,
                              const G4double beta, 
...
G4double OKDsG4Cerenkov::GetPoolPmtQe(G4double energy) const
{
  ...
  if(first) {
    ...
    if( bialkali ==0 ) {
      G4cout<<"Error: OKDsG4Cerenkov::Can't find material bialkali."<<G4endl;
    }
    first = false;
  }
