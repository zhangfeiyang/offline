...
DAEDsG4Cerenkov::DAEDsG4Cerenkov(const G4String& processName, G4ProcessType type)
           : G4VProcess(processName, type)
           , fApplyPreQE(false)
           , fPreQE(1.)
{
        G4cout << "DAEDsG4Cerenkov::DAEDsG4Cerenkov constructor" << G4endl;
...
DAEDsG4Cerenkov::~DAEDsG4Cerenkov() 
{
...
//
G4VParticleChange*
DAEDsG4Cerenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
	...
	if (verboseLevel>0) {
	  G4cout << "DAEDsG4Cerenkov MeanNumberOfPhotons "<< MeanNumberOfPhotons 
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if (not daemgr) {
        ...
        ToolBase* t = 0;
        std::string new_name = "G4DAEAnaMgr";
        t = detsimalg->findTool(new_name);
        if (not t) {
            assert (t);
        }
        daemgr = dynamic_cast<G4DAEAnaMgr*>(t);
    }
    // == get the Step List ==
    G4DAECerenkovStepList* csl = daemgr->getCSL();
...
///////////////////////////////////////////////////////////////////////////////
	...
	for (G4int i = 0; i < NumPhotons; i++) {
	  ...
	  if (verboseLevel>0) {
	    G4cout << "DAEDsG4Cerenkov  aSecondaryTrack->SetWeight( fPhotonWeight="<<fPhotonWeight<<" * aTrack.GetWeight()= " << aTrack.GetWeight() 
	...
	if (verboseLevel>0) {
	G4cout << "\n Exiting from DAEDsG4Cerenkov::DoIt -- NumberOfSecondaries = " 
...
void DAEDsG4Cerenkov::BuildThePhysicsTable()
{
...
G4double DAEDsG4Cerenkov::GetMeanFreePath(const G4Track&,
                                           G4double,
                                           G4ForceCondition*)
{
        return 1.;
...
G4double DAEDsG4Cerenkov::PostStepGetPhysicalInteractionLength(
...
G4double 
DAEDsG4Cerenkov::GetAverageNumberOfPhotons(const G4double charge,
                              const G4double beta, 
...
G4double DAEDsG4Cerenkov::GetPoolPmtQe(G4double energy) const
{
  ...
  if(first) {
    ...
    if( bialkali ==0 ) {
      G4cout<<"Error: DAEDsG4Cerenkov::Can't find material bialkali."<<G4endl;
    }
    first = false;
  }
