...
#cmt_local_tagfile_install_more_includes = $(GenTools_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GenTools_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GenTools_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GenToolssetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GenTools" = ""; then \
	    if test -d ../GenTools; then \
	      echo "Installing files from more ../GenTools to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GenTools; then mkdir -p $(install_include_dir)/GenTools; fi;\
	        $(install_command) ../GenTools $(install_include_dir);\
	    else \
	      echo "No more GenTools include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GenTools"; \
	  $(uninstall_command) "$(install_include_dir)/GenTools/*" ; \
	  if test -f $(install_include_dir)/GenTools.cmtref ; then \
	    echo "Removing $(install_include_dir)/GenTools.cmtref"; \
	    eval rm -f $(install_include_dir)/GenTools.cmtref ; \
	  fi; \
	fi
