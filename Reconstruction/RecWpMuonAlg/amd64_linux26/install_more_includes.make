...
#cmt_local_tagfile_install_more_includes = $(RecWpMuonAlg_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RecWpMuonAlg_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RecWpMuonAlg_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RecWpMuonAlgsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RecWpMuonAlg" = ""; then \
	    if test -d ../RecWpMuonAlg; then \
	      echo "Installing files from more ../RecWpMuonAlg to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RecWpMuonAlg; then mkdir -p $(install_include_dir)/RecWpMuonAlg; fi;\
	        $(install_command) ../RecWpMuonAlg $(install_include_dir);\
	    else \
	      echo "No more RecWpMuonAlg include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RecWpMuonAlg"; \
	  $(uninstall_command) "$(install_include_dir)/RecWpMuonAlg/*" ; \
	  if test -f $(install_include_dir)/RecWpMuonAlg.cmtref ; then \
	    echo "Removing $(install_include_dir)/RecWpMuonAlg.cmtref"; \
	    eval rm -f $(install_include_dir)/RecWpMuonAlg.cmtref ; \
	  fi; \
	fi
