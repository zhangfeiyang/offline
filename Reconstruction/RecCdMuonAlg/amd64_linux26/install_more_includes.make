...
#cmt_local_tagfile_install_more_includes = $(RecCdMuonAlg_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RecCdMuonAlg_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RecCdMuonAlg_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RecCdMuonAlgsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RecCdMuonAlg" = ""; then \
	    if test -d ../RecCdMuonAlg; then \
	      echo "Installing files from more ../RecCdMuonAlg to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RecCdMuonAlg; then mkdir -p $(install_include_dir)/RecCdMuonAlg; fi;\
	        $(install_command) ../RecCdMuonAlg $(install_include_dir);\
	    else \
	      echo "No more RecCdMuonAlg include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RecCdMuonAlg"; \
	  $(uninstall_command) "$(install_include_dir)/RecCdMuonAlg/*" ; \
	  if test -f $(install_include_dir)/RecCdMuonAlg.cmtref ; then \
	    echo "Removing $(install_include_dir)/RecCdMuonAlg.cmtref"; \
	    eval rm -f $(install_include_dir)/RecCdMuonAlg.cmtref ; \
	  fi; \
	fi
