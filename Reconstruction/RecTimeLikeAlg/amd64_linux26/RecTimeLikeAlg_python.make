#-- start of make_header -----------------

#====================================
#  Document RecTimeLikeAlg_python
#
#   Generated Fri Jul 10 19:20:16 2020  by zhangfy
#
#====================================

include ${CMTROOT}/src/Makefile.core

ifdef tag
CMTEXTRATAGS = $(tag)
else
tag       = $(CMTCONFIG)
endif

cmt_RecTimeLikeAlg_python_has_no_target_tag = 1

#--------------------------------------------------------

ifdef cmt_RecTimeLikeAlg_python_has_target_tag

tags      = $(tag),$(CMTEXTRATAGS),target_RecTimeLikeAlg_python

RecTimeLikeAlg_tag = $(tag)

#cmt_local_tagfile_RecTimeLikeAlg_python = $(RecTimeLikeAlg_tag)_RecTimeLikeAlg_python.make
cmt_local_tagfile_RecTimeLikeAlg_python = $(bin)$(RecTimeLikeAlg_tag)_RecTimeLikeAlg_python.make

else

tags      = $(tag),$(CMTEXTRATAGS)

RecTimeLikeAlg_tag = $(tag)

#cmt_local_tagfile_RecTimeLikeAlg_python = $(RecTimeLikeAlg_tag).make
cmt_local_tagfile_RecTimeLikeAlg_python = $(bin)$(RecTimeLikeAlg_tag).make

endif

include $(cmt_local_tagfile_RecTimeLikeAlg_python)
#-include $(cmt_local_tagfile_RecTimeLikeAlg_python)

ifdef cmt_RecTimeLikeAlg_python_has_target_tag

cmt_final_setup_RecTimeLikeAlg_python = $(bin)setup_RecTimeLikeAlg_python.make
cmt_dependencies_in_RecTimeLikeAlg_python = $(bin)dependencies_RecTimeLikeAlg_python.in
#cmt_final_setup_RecTimeLikeAlg_python = $(bin)RecTimeLikeAlg_RecTimeLikeAlg_pythonsetup.make
cmt_local_RecTimeLikeAlg_python_makefile = $(bin)RecTimeLikeAlg_python.make

else

cmt_final_setup_RecTimeLikeAlg_python = $(bin)setup.make
cmt_dependencies_in_RecTimeLikeAlg_python = $(bin)dependencies.in
#cmt_final_setup_RecTimeLikeAlg_python = $(bin)RecTimeLikeAlgsetup.make
cmt_local_RecTimeLikeAlg_python_makefile = $(bin)RecTimeLikeAlg_python.make

endif

#cmt_final_setup = $(bin)setup.make
#cmt_final_setup = $(bin)RecTimeLikeAlgsetup.make

#RecTimeLikeAlg_python :: ;

dirs ::
	@if test ! -r requirements ; then echo "No requirements file" ; fi; \
	  if test ! -d $(bin) ; then $(mkdir) -p $(bin) ; fi

javadirs ::
	@if test ! -d $(javabin) ; then $(mkdir) -p $(javabin) ; fi

srcdirs ::
	@if test ! -d $(src) ; then $(mkdir) -p $(src) ; fi

help ::
	$(echo) 'RecTimeLikeAlg_python'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = RecTimeLikeAlg_python/
#RecTimeLikeAlg_python::
#	@if test ! -d $(bin)$(binobj) ; then $(mkdir) -p $(bin)$(binobj) ; fi
#	$(echo) "STRUCTURED_OUTPUT="$(bin)$(binobj)
endif

${CMTROOT}/src/Makefile.core : ;
ifdef use_requirements
$(use_requirements) : ;
endif

#-- end of make_header ------------------
#-- start of install_python_header ------


installarea = ${CMTINSTALLAREA}
install_python_dir = $(installarea)

ifneq ($(strip "$(source)"),"")
src = ../$(source)
dest = $(install_python_dir)/python
else
src = ../python
dest = $(install_python_dir)
endif

ifneq ($(strip "$(offset)"),"")
dest = $(install_python_dir)/python
endif

RecTimeLikeAlg_python :: RecTimeLikeAlg_pythoninstall

install :: RecTimeLikeAlg_pythoninstall

RecTimeLikeAlg_pythoninstall :: $(install_python_dir)
	@if [ ! "$(installarea)" = "" ] ; then\
	  echo "installation done"; \
	fi

$(install_python_dir) ::
	@if [ "$(installarea)" = "" ] ; then \
	  echo "Cannot install header files, no installation source specified"; \
	else \
	  if [ -d $(src) ] ; then \
	    echo "Installing files from $(src) to $(dest)" ; \
	    if [ "$(offset)" = "" ] ; then \
	      $(install_command) --exclude="*.py?" $(src) $(dest) ; \
	    else \
	      $(install_command) --exclude="*.py?" $(src) $(dest) --destname $(offset); \
	    fi ; \
	  else \
	    echo "no source  $(src)"; \
	  fi; \
	fi

RecTimeLikeAlg_pythonclean :: RecTimeLikeAlg_pythonuninstall

uninstall :: RecTimeLikeAlg_pythonuninstall

RecTimeLikeAlg_pythonuninstall ::
	@if test "$(installarea)" = ""; then \
	  echo "Cannot uninstall header files, no installation source specified"; \
	else \
	  echo "Uninstalling files from $(dest)"; \
	  $(uninstall_command) "$(dest)" ; \
	fi


#-- end of install_python_header ------
#-- start of cleanup_header --------------

clean :: RecTimeLikeAlg_pythonclean ;
#	@cd .

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(RecTimeLikeAlg_python.make) $@: No rule for such target" >&2
else
.DEFAULT::
	$(error PEDANTIC: $@: No rule for such target)
endif

RecTimeLikeAlg_pythonclean ::
#-- end of cleanup_header ---------------
