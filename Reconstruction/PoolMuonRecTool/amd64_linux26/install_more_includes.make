...
#cmt_local_tagfile_install_more_includes = $(PoolMuonRecTool_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(PoolMuonRecTool_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)PoolMuonRecTool_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)PoolMuonRecToolsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "PoolMuonRecTool" = ""; then \
	    if test -d ../PoolMuonRecTool; then \
	      echo "Installing files from more ../PoolMuonRecTool to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/PoolMuonRecTool; then mkdir -p $(install_include_dir)/PoolMuonRecTool; fi;\
	        $(install_command) ../PoolMuonRecTool $(install_include_dir);\
	    else \
	      echo "No more PoolMuonRecTool include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/PoolMuonRecTool"; \
	  $(uninstall_command) "$(install_include_dir)/PoolMuonRecTool/*" ; \
	  if test -f $(install_include_dir)/PoolMuonRecTool.cmtref ; then \
	    echo "Removing $(install_include_dir)/PoolMuonRecTool.cmtref"; \
	    eval rm -f $(install_include_dir)/PoolMuonRecTool.cmtref ; \
	  fi; \
	fi
