...
#cmt_local_tagfile_install_more_includes = $(ConeMuonRecTool_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(ConeMuonRecTool_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)ConeMuonRecTool_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)ConeMuonRecToolsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "ConeMuonRecTool" = ""; then \
	    if test -d ../ConeMuonRecTool; then \
	      echo "Installing files from more ../ConeMuonRecTool to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/ConeMuonRecTool; then mkdir -p $(install_include_dir)/ConeMuonRecTool; fi;\
	        $(install_command) ../ConeMuonRecTool $(install_include_dir);\
	    else \
	      echo "No more ConeMuonRecTool include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/ConeMuonRecTool"; \
	  $(uninstall_command) "$(install_include_dir)/ConeMuonRecTool/*" ; \
	  if test -f $(install_include_dir)/ConeMuonRecTool.cmtref ; then \
	    echo "Removing $(install_include_dir)/ConeMuonRecTool.cmtref"; \
	    eval rm -f $(install_include_dir)/ConeMuonRecTool.cmtref ; \
	  fi; \
	fi
