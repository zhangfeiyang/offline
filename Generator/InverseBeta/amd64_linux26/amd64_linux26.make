...
package=InverseBeta
version=v0
PACKAGE_ROOT=$(INVERSEBETAROOT)
srcdir=../src
bin=../$(InverseBeta_tag)/
javabin=../classes/
mgrdir=cmt
BIN=/junofs/production/public/users/zhangfy/offline_bgd/Generator/InverseBeta/amd64_linux26/
project=offline
...
use_linkopts= $(cmt_installarea_linkopts)   $(InverseBeta_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
...
use_stamps=  $(InverseBeta_stamps)  $(ROOT_stamps)  $(CLHEP_stamps) 
use_cflags=  $(InverseBeta_cflags)  $(ROOT_cflags)  $(CLHEP_cflags) 
use_pp_cflags=  $(InverseBeta_pp_cflags)  $(ROOT_pp_cflags)  $(CLHEP_pp_cflags) 
use_cppflags=  $(InverseBeta_cppflags)  $(ROOT_cppflags)  $(CLHEP_cppflags) 
use_pp_cppflags=  $(InverseBeta_pp_cppflags)  $(ROOT_pp_cppflags)  $(CLHEP_pp_cppflags) 
use_fflags=  $(InverseBeta_fflags)  $(ROOT_fflags)  $(CLHEP_fflags) 
use_pp_fflags=  $(InverseBeta_pp_fflags)  $(ROOT_pp_fflags)  $(CLHEP_pp_fflags) 
use_libraries= $(ROOT_libraries)  $(CLHEP_libraries) 
includes= $(ppcmd)"$(srcdir)" $(use_includes)
fincludes= $(includes)
IBD_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
IBD_use_linkopts=  $(InverseBeta_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
make_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
constituents= IBD 
all_constituents= $(constituents)
constituentsclean= IBDclean 
all_constituentsclean= $(constituentsclean)
cmt_actions_constituents= make 
cmt_actions_constituentsclean= makeclean 
IBDprototype_dependencies= $(IBDcompile_dependencies)
