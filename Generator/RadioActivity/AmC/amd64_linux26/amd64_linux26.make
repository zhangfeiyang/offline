...
package=AmC
version=v0
PACKAGE_ROOT=$(AMCROOT)
srcdir=../src
bin=../$(AmC_tag)/
javabin=../classes/
mgrdir=cmt
BIN=/junofs/production/public/users/zhangfy/offline_bgd/Generator/RadioActivity/AmC/amd64_linux26/
project=offline
...
use_linkopts= $(cmt_installarea_linkopts)   $(AmC_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
...
use_stamps=  $(AmC_stamps)  $(ROOT_stamps)  $(CLHEP_stamps) 
use_cflags=  $(AmC_cflags)  $(ROOT_cflags)  $(CLHEP_cflags) 
use_pp_cflags=  $(AmC_pp_cflags)  $(ROOT_pp_cflags)  $(CLHEP_pp_cflags) 
use_cppflags=  $(AmC_cppflags)  $(ROOT_cppflags)  $(CLHEP_cppflags) 
use_pp_cppflags=  $(AmC_pp_cppflags)  $(ROOT_pp_cppflags)  $(CLHEP_pp_cppflags) 
use_fflags=  $(AmC_fflags)  $(ROOT_fflags)  $(CLHEP_fflags) 
use_pp_fflags=  $(AmC_pp_fflags)  $(ROOT_pp_fflags)  $(CLHEP_pp_fflags) 
use_libraries= $(ROOT_libraries)  $(CLHEP_libraries) 
includes= $(ppcmd)"$(srcdir)" $(use_includes)
fincludes= $(includes)
AmC_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
AmC_use_linkopts=  $(AmC_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
make_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
constituents= AmC 
all_constituents= $(constituents)
constituentsclean= AmCclean 
all_constituentsclean= $(constituentsclean)
cmt_actions_constituents= make 
cmt_actions_constituentsclean= makeclean 
AmCprototype_dependencies= $(AmCcompile_dependencies)
