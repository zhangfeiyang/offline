...
#cmt_local_tagfile = $(Identifier_tag).make
cmt_local_tagfile = $(bin)$(Identifier_tag).make
...
cmt_local_tagfile_install_more_includes = $(bin)$(Identifier_tag)_install_more_includes.make
...
cmt_local_tagfile_install_more_includes = $(bin)$(Identifier_tag).make
...
Identifier_extratags = -tag_add=target_Identifier
...
not_Identifiercompile_dependencies = { n=0; for p in $?; do m=0; for d in $(Identifiercompile_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
Identifierdirs :
	@if test ! -d $(bin)Identifier; then $(mkdir) -p $(bin)Identifier; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)Identifier
else
Identifierdirs : ;
endif
...
ifndef QUICK
$(cmt_local_Identifier_makefile) : $(Identifiercompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building Identifier.make"; \
	  $(cmtexe) -tag=$(tags) $(Identifier_extratags) build constituent_config -out=$(cmt_local_Identifier_makefile) Identifier
else
$(cmt_local_Identifier_makefile) : $(Identifiercompile_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_Identifier) ] || \
	  [ ! -f $(cmt_final_setup_Identifier) ] || \
	  $(not_Identifiercompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building Identifier.make"; \
	  $(cmtexe) -tag=$(tags) $(Identifier_extratags) build constituent_config -out=$(cmt_local_Identifier_makefile) Identifier; \
	  fi
endif
...
$(cmt_local_Identifier_makefile) : $(Identifiercompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building Identifier.make"; \
	  $(cmtexe) -f=$(bin)Identifier.in -tag=$(tags) $(Identifier_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_Identifier_makefile) Identifier
else
$(cmt_local_Identifier_makefile) : $(Identifiercompile_dependencies) $(cmt_build_library_linksstamp) $(bin)Identifier.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_Identifier) ] || \
	  [ ! -f $(cmt_final_setup_Identifier) ] || \
	  $(not_Identifiercompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(Identifier_extratags) build constituent_makefile -out=$(cmt_local_Identifier_makefile) Identifier

Identifier :: Identifiercompile Identifierinstall ;

ifdef cmt_Identifier_has_prototypes

Identifierprototype : $(Identifierprototype_dependencies) $(cmt_local_Identifier_makefile) dirs Identifierdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_Identifier_makefile); then \
	  $(MAKE) -f $(cmt_local_Identifier_makefile) $@; \
	  fi
#	@$(MAKE) -f $(cmt_local_Identifier_makefile) $@
	$(echo) "(constituents.make) $@ done"

Identifiercompile : Identifierprototype

endif

Identifiercompile : $(Identifiercompile_dependencies) $(cmt_local_Identifier_makefile) dirs Identifierdirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_Identifier_makefile); then \
	  $(MAKE) -f $(cmt_local_Identifier_makefile) $@; \
	  fi
...
Identifierclean :: $(Identifierclean_dependencies) ##$(cmt_local_Identifier_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_Identifier_makefile); then \
	  $(MAKE) -f $(cmt_local_Identifier_makefile) $@; \
	fi
	$(echo) "(constituents.make) $@ done"
#	@-$(MAKE) -f $(cmt_local_Identifier_makefile) Identifierclean

##	  /bin/rm -f $(cmt_local_Identifier_makefile) $(bin)Identifier_dependencies.make

install :: Identifierinstall ;

Identifierinstall :: Identifiercompile $(Identifier_dependencies) $(cmt_local_Identifier_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_Identifier_makefile); then \
	  $(MAKE) -f $(cmt_local_Identifier_makefile) $@; \
	  fi
...
$(foreach d,$(Identifier_dependencies),$(eval $(d)uninstall_dependencies += Identifieruninstall))

Identifieruninstall : $(Identifieruninstall_dependencies) ##$(cmt_local_Identifier_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_Identifier_makefile); then \
	  $(MAKE) -f $(cmt_local_Identifier_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_Identifier_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: Identifieruninstall ;

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(constituents.make) Starting $@ Identifier"
	$(echo) Using default action for $@
	$(echo) "(constituents.make) $@ Identifier done"
endif
...
cmt_local_tagfile_make = $(bin)$(Identifier_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(Identifier_tag).make
