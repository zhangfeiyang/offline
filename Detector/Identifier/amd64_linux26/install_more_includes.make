...
#cmt_local_tagfile_install_more_includes = $(Identifier_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(Identifier_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)Identifier_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)Identifiersetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "Identifier" = ""; then \
	    if test -d ../Identifier; then \
	      echo "Installing files from more ../Identifier to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/Identifier; then mkdir -p $(install_include_dir)/Identifier; fi;\
	        $(install_command) ../Identifier $(install_include_dir);\
	    else \
	      echo "No more Identifier include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/Identifier"; \
	  $(uninstall_command) "$(install_include_dir)/Identifier/*" ; \
	  if test -f $(install_include_dir)/Identifier.cmtref ; then \
	    echo "Removing $(install_include_dir)/Identifier.cmtref"; \
	    eval rm -f $(install_include_dir)/Identifier.cmtref ; \
	  fi; \
	fi
