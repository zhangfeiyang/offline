...
#cmt_local_tagfile_install_more_includes = $(Geometry_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(Geometry_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)Geometry_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)Geometrysetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
	...
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "Geometry" = ""; then \
	    if test -d ../Geometry; then \
	      echo "Installing files from more ../Geometry to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/Geometry; then mkdir -p $(install_include_dir)/Geometry; fi;\
	        $(install_command) ../Geometry $(install_include_dir);\
	    else \
	      echo "No more Geometry include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/Geometry"; \
	  $(uninstall_command) "$(install_include_dir)/Geometry/*" ; \
	  if test -f $(install_include_dir)/Geometry.cmtref ; then \
	    echo "Removing $(install_include_dir)/Geometry.cmtref"; \
	    eval rm -f $(install_include_dir)/Geometry.cmtref ; \
	  fi; \
	fi
