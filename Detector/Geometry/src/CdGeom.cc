...
PmtGeom* CdGeom::getPmt(Identifier id)
{
    std::map<Identifier, PmtGeom*>::iterator it = m_mapIdToPmt.find(id);
    if ( it == m_mapIdToPmt.end() ) {
        std::cerr << "id " << id << "(" << CdID::module(id) << ", " << CdID::pmt(id)
...
{  
    std::cout << "centerDet " << __func__ << " begin " << getVerb() << std::endl;
    ...
    std::cout << "centerDEt " << __func__ << " end " << getVerb() << std::endl;
}
...
void CdGeom::printCd()
{
    std::cout << "CenterDet " << __func__ << " begin" << std::endl;
 
    if (m_AcrylicIsComposite) {
	std::cout << "CenterDetAcrylicBall Rmax = " << getCdBallRmax()       << std::endl
	          << "CenterDetAcrylicBall Rmin = " << getCdBallRmin()       << std::endl
		  << "CenterDetChimney Rmax = "     << getCdChimneyRmax()    << std::endl
		  << "CenterDetChimney Rmin = "     << getCdChimneyRmin()    << std::endl
		  << "CenterDetChimney Dz = "       << getCdChimneyDz()      << std::endl
		  << "CenterDetChimneyPos x = "     << getChimneyTrans()[0]  << ",y = " << getChimneyTrans()[1] << ",z = " << getChimneyTrans()[2] << std::endl;
    }
    else {
	std::cout << "CenterDetAcrylicBall Rmax = " << getCdBallRmax()       << std::endl
		  << "CenterDetAcrylicBall Rmin = " << getCdBallRmin()       << std::endl;
    ...
    std::cout << "CenterDet " << __func__ << " end" << std::endl;
} 
