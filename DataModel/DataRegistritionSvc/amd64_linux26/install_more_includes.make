...
#cmt_local_tagfile_install_more_includes = $(DataRegistritionSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(DataRegistritionSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)DataRegistritionSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)DataRegistritionSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "DataRegistritionSvc" = ""; then \
	    if test -d ../DataRegistritionSvc; then \
	      echo "Installing files from more ../DataRegistritionSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/DataRegistritionSvc; then mkdir -p $(install_include_dir)/DataRegistritionSvc; fi;\
	        $(install_command) ../DataRegistritionSvc $(install_include_dir);\
	    else \
	      echo "No more DataRegistritionSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/DataRegistritionSvc"; \
	  $(uninstall_command) "$(install_include_dir)/DataRegistritionSvc/*" ; \
	  if test -f $(install_include_dir)/DataRegistritionSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/DataRegistritionSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/DataRegistritionSvc.cmtref ; \
	  fi; \
	fi
