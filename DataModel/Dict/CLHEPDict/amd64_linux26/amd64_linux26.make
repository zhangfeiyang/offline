...
CLHEPDict_shlibflags=$(libraryshr_linkopts) $(cmt_installarea_linkopts) $(CLHEPDict_use_linkopts)
CLHEPDict_use_linkopts=    $(SniperPolicy_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
CLHEPDict_stamps=${CLHEPDICTROOT}/${BINDIR}/CLHEPDict.stamp 
CLHEPDict_linker_library=CLHEPDict
tag=amd64_linux26
package=CLHEPDict
version=v0
PACKAGE_ROOT=$(CLHEPDICTROOT)
srcdir=../src
bin=../$(CLHEPDict_tag)/
javabin=../classes/
mgrdir=cmt
BIN=/junofs/production/public/users/zhangfy/offline_bgd/DataModel/Dict/CLHEPDict/amd64_linux26/
project=offline
...
use_linkopts= $(cmt_installarea_linkopts)   $(CLHEPDict_linkopts)  $(SniperPolicy_linkopts)  $(ROOT_linkopts)  $(CLHEP_linkopts) 
...
use_requirements=requirements $(CMT_root)/mgr/requirements $(XmlObjDesc_root)/cmt/requirements $(SniperPolicy_root)/cmt/requirements $(ROOT_root)/cmt/requirements $(CLHEP_root)/cmt/requirements 
use_includes= $(ppcmd)"$(SniperPolicy_root)/src" $(ppcmd)"/cvmfs/juno.ihep.ac.cn/sl6_amd64_gcc447/Pre-Release/J18v1r1-Pre1/ExternalLibs/ROOT/5.34.11/include" $(ppcmd)"$(CLHEP_root)/src" 
use_fincludes= $(use_includes)
use_stamps=  $(CLHEPDict_stamps)  $(SniperPolicy_stamps)  $(ROOT_stamps)  $(CLHEP_stamps) 
use_cflags=  $(CLHEPDict_cflags)  $(SniperPolicy_cflags)  $(ROOT_cflags)  $(CLHEP_cflags) 
use_pp_cflags=  $(CLHEPDict_pp_cflags)  $(SniperPolicy_pp_cflags)  $(ROOT_pp_cflags)  $(CLHEP_pp_cflags) 
use_cppflags=  $(CLHEPDict_cppflags)  $(SniperPolicy_cppflags)  $(ROOT_cppflags)  $(CLHEP_cppflags) 
use_pp_cppflags=  $(CLHEPDict_pp_cppflags)  $(SniperPolicy_pp_cppflags)  $(ROOT_pp_cppflags)  $(CLHEP_pp_cppflags) 
use_fflags=  $(CLHEPDict_fflags)  $(SniperPolicy_fflags)  $(ROOT_fflags)  $(CLHEP_fflags) 
use_pp_fflags=  $(CLHEPDict_pp_fflags)  $(SniperPolicy_pp_fflags)  $(ROOT_pp_fflags)  $(CLHEP_pp_fflags) 
use_libraries= $(XmlObjDesc_libraries)  $(SniperPolicy_libraries)  $(ROOT_libraries)  $(CLHEP_libraries) 
...
fincludes= $(includes)
CLHEPDictDict_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
CLHEPDict_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
make_GUID={88BF15AB-5A2D-4bea-B64F-02752C2A1F4F}
constituents= CLHEPDictDict CLHEPDict 
all_constituents= $(constituents)
constituentsclean= CLHEPDictclean CLHEPDictDictclean 
all_constituentsclean= $(constituentsclean)
cmt_actions_constituents= make 
cmt_actions_constituentsclean= makeclean 
CLHEPDictcompile_dependencies= CLHEPDictDict
CLHEPDictprototype_dependencies= $(CLHEPDictcompile_dependencies)
