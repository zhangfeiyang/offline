...
#
#   Generated Fri Jul 10 19:17:55 2020  by zhangfy
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(EvtNavigator_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(EvtNavigator_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)EvtNavigator_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)EvtNavigatorsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "EvtNavigator" = ""; then \
	    if test -d ../EvtNavigator; then \
	      echo "Installing files from more ../EvtNavigator to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/EvtNavigator; then mkdir -p $(install_include_dir)/EvtNavigator; fi;\
	        $(install_command) ../EvtNavigator $(install_include_dir);\
	    else \
	      echo "No more EvtNavigator include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/EvtNavigator"; \
	  $(uninstall_command) "$(install_include_dir)/EvtNavigator/*" ; \
	  if test -f $(install_include_dir)/EvtNavigator.cmtref ; then \
	    echo "Removing $(install_include_dir)/EvtNavigator.cmtref"; \
	    eval rm -f $(install_include_dir)/EvtNavigator.cmtref ; \
	  fi; \
	fi
