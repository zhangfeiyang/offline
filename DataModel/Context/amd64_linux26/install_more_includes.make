...
#cmt_local_tagfile_install_more_includes = $(Context_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(Context_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)Context_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)Contextsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
	...
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "Context" = ""; then \
	    if test -d ../Context; then \
	      echo "Installing files from more ../Context to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/Context; then mkdir -p $(install_include_dir)/Context; fi;\
	        $(install_command) ../Context $(install_include_dir);\
	    else \
	      echo "No more Context include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/Context"; \
	  $(uninstall_command) "$(install_include_dir)/Context/*" ; \
	  if test -f $(install_include_dir)/Context.cmtref ; then \
	    echo "Removing $(install_include_dir)/Context.cmtref"; \
	    eval rm -f $(install_include_dir)/Context.cmtref ; \
	  fi; \
	fi
