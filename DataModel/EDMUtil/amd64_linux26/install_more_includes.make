...
#
#   Generated Fri Jul 10 19:17:38 2020  by zhangfy
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(EDMUtil_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(EDMUtil_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)EDMUtil_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)EDMUtilsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "EDMUtil" = ""; then \
	    if test -d ../EDMUtil; then \
	      echo "Installing files from more ../EDMUtil to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/EDMUtil; then mkdir -p $(install_include_dir)/EDMUtil; fi;\
	        $(install_command) ../EDMUtil $(install_include_dir);\
	    else \
	      echo "No more EDMUtil include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/EDMUtil"; \
	  $(uninstall_command) "$(install_include_dir)/EDMUtil/*" ; \
	  if test -f $(install_include_dir)/EDMUtil.cmtref ; then \
	    echo "Removing $(install_include_dir)/EDMUtil.cmtref"; \
	    eval rm -f $(install_include_dir)/EDMUtil.cmtref ; \
	  fi; \
	fi
