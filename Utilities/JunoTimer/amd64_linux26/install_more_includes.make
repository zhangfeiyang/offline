...
#cmt_local_tagfile_install_more_includes = $(JunoTimer_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(JunoTimer_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)JunoTimer_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)JunoTimersetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "JunoTimer" = ""; then \
	    if test -d ../JunoTimer; then \
	      echo "Installing files from more ../JunoTimer to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/JunoTimer; then mkdir -p $(install_include_dir)/JunoTimer; fi;\
	        $(install_command) ../JunoTimer $(install_include_dir);\
	    else \
	      echo "No more JunoTimer include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/JunoTimer"; \
	  $(uninstall_command) "$(install_include_dir)/JunoTimer/*" ; \
	  if test -f $(install_include_dir)/JunoTimer.cmtref ; then \
	    echo "Removing $(install_include_dir)/JunoTimer.cmtref"; \
	    eval rm -f $(install_include_dir)/JunoTimer.cmtref ; \
	  fi; \
	fi
