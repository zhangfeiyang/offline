...
#cmt_local_tagfile_install_more_includes = $(InputReviser_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(InputReviser_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)InputReviser_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)InputRevisersetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "InputReviser" = ""; then \
	    if test -d ../InputReviser; then \
	      echo "Installing files from more ../InputReviser to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/InputReviser; then mkdir -p $(install_include_dir)/InputReviser; fi;\
	        $(install_command) ../InputReviser $(install_include_dir);\
	    else \
	      echo "No more InputReviser include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/InputReviser"; \
	  $(uninstall_command) "$(install_include_dir)/InputReviser/*" ; \
	  if test -f $(install_include_dir)/InputReviser.cmtref ; then \
	    echo "Removing $(install_include_dir)/InputReviser.cmtref"; \
	    eval rm -f $(install_include_dir)/InputReviser.cmtref ; \
	  fi; \
	fi
